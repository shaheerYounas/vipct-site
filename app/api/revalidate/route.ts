...
import { languages, publicSlugs } from "@/lib/site-data";
...
export async function POST(request: NextRequest) {
  ...
  if (slug && lang && Object.prototype.hasOwnProperty.call(languages, lang)) {
    ...
    if (lang === "en") revalidatePath(`/${slug}`);
    return NextResponse.json({ revalidated: [`/${lang}/${slug}`] });
  }

  for (const language of Object.keys(languages) as Language[]) {
    for (const pageSlug of publicSlugs) revalidatePath(`/${language}/${pageSlug}`);
  }
  publicSlugs.forEach((pageSlug) => revalidatePath(`/${pageSlug}`));
